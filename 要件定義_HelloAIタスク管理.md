...
- ガントチャート、工数集計
- 3つ目以降のロール（閲覧専用など）
- Gmail / Slack / Googleカレンダー連携によるToDo自動抽出＋AIエージェントによるタスク自動処理（詳細は12章）
...
- CLAUDE.mdの明示的禁止事項（Slack通知・メール通知・リアルタイム同期）と重なる領域であり、既存の権限設計（RLS・admin/member）にも影響が及ぶため、今回のデータモデル（`profiles`/`tasks`の2テーブルのみ）を拡張せずに済む範囲に留める